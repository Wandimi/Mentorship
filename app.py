...
def preview():
    demo_user = SimpleNamespace(
        name="Alex Morgan",
        role="mentor",
        bio="Product leader focused on growth, career transitions, and leadership development.",
        skills="Product Strategy,Leadership,Coaching,UX Research",
        availability="Tuesdays & Thursdays",
    )
    mentors = [
        SimpleNamespace(
            name="Jordan Lee",
            skills="Engineering Leadership,Team Scaling,Architecture",
            availability="Fridays",
        ),
        SimpleNamespace(
            name="Priya Patel",
            skills="Data Science,AI Strategy,Storytelling",
            availability="Mondays",
        ),
    ]
    demo_requests = [
        SimpleNamespace(
            goal="Prepare for a first-time manager role",
            mentor=SimpleNamespace(name="Alex Morgan"),
            mentee=SimpleNamespace(name="Sam Rivera"),
            status="accepted",
        ),
        SimpleNamespace(
            goal="Build a roadmap for a new fintech product",
            mentor=SimpleNamespace(name="Jordan Lee"),
            mentee=SimpleNamespace(name="Alex Morgan"),
            status="pending",
        ),
    ]
    stats = SimpleNamespace(total_users=128, mentor_count=58, mentee_count=70, active_mentorships=24)
    return render_template(
        "dashboard.html",
        current_user=demo_user,
        mentors=mentors,
        my_requests=demo_requests,
        stats=stats,
    )
